...
# Each scene in the validation set contains a list of images, poses, and pairs.
# Let's pick one and look at some images.

scene = 'piazza_san_marco'

images_dict = read_images(src, scene)

print(f'Loaded {len(images_dict)} images.')

# disp_multiple_pics(images_dict)


covisibility_dict = read_covisibility_data(src, scene)

# disp_covisibility_sample(covisibility_dict, images_dict)

keys = list(images_dict.keys())
image = images_dict[keys[0]]
keypoints, descriptors = sift_get_local_features(image)

disp_keypoints(image, keypoints)
...
keypoints_1, descriptors_1 = sift_get_local_features(images_dict[image_id_1], 2000)
keypoints_2, descriptors_2 = sift_get_local_features(images_dict[image_id_2], 2000)
...
calib_dict = read_calibration_dict(src, scene)
...
for scene in scaling_dict.keys():
    ...
    for id in tqdm(ids):
        ...
        kp_dict[id], desc_dict[id] = sift_get_local_features(images_dict[id], num_features=2000)
        #ExtractSiftFeatures(images_dict[id], sift_detector, 2000)
    print()
